...
class Upgrader {
  static run(creep) {
    function tryLink() {
      const link = helpers_find_1.HelpersFind.findClosestStructureByPathFromArray(
        creep.pos,
        creep.room,
        helpers_find_1.HelpersFind.findLinksWithEnergy(creep.room)
      );
      if (link === null) {
        helpers_creep_1.HelpersCreep.logError(
          creep,
          'no link with energy in room found'
        );
        return false;
      }
    ...
    function tryStorage() {
      ...
      if (storage === undefined || storage.store[RESOURCE_ENERGY] === 0) {
        helpers_creep_1.HelpersCreep.logError(
          creep,
          'no storage with energy in room found'
        );
        return false;
      }
