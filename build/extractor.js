...
class Extractor {
  static run(creep) {
    ...
    if (
      ...
      helpers_creep_1.HelpersCreep.totalCarry(creep) === 0
    ) {
    ...
    if (creep.memory.transferring) {
      ...
      //             }
      //     });
      const terminals = helpers_1.HelpersFind.findStructuresByType(
        creep.room,
        STRUCTURE_TERMINAL
      );
      if (terminals.length === 1) {
        ...
        const myResource =
          creep.carry[RESOURCE_HYDROGEN] !== undefined
            ? RESOURCE_HYDROGEN
            : RESOURCE_KEANIUM;
        ...
        if (transferResult === ERR_NOT_IN_RANGE) {
          ...
          helpers_creep_1.HelpersCreep.logError(
            creep,
        ...
        helpers_creep_1.HelpersCreep.logError(
          creep,
          'no terminal in room found'
        );
      }
    } else {
      const mineral = helpers_1.HelpersFind.findClosestPath(
        creep.pos,
        creep.room,
        FIND_MINERALS
      );
