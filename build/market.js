...
class Market {
  constructor(room, game = Game) {
    ...
    this.game = game;
    this.terminals = helpers_1.HelpersFind.findMyActiveReadyToUseTerminals(
      room
    );
  }
  run() {
  ...
  terminalTrade(terminal) {
    const resourcesAndAmounts = helpers_1.HelpersTerminal.getTerminalMarketResourcesAndAmounts(
      terminal
    );
    ...
    for (const resourceAndAmount of resourcesAndAmounts) {
      ...
      const theBestOrder = helpers_1.HelpersTerminal.pickTheBestOrder(
        this.roomName,
        sortedOrders,
        availableAmount,
        energy
      ...
      if (theBestOrder !== false) {
        helpers_1.HelpersTerminal.makeTrade(
          theBestOrder.id,
          availableAmount,
          theBestOrder.amount,
          this.roomName
