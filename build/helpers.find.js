...
class HelpersFind {
  ...
  static findMyActiveReadyToUseTerminals(room) {
    return HelpersFind.findStructuresByType(room, STRUCTURE_TERMINAL).filter(
      terminal =>
  ...
  static getRoomTotalEnergyForSpawning(room) {
    ...
    const extensions = HelpersFind.findStructuresByType(
      room,
      STRUCTURE_EXTENSION
    );
  ...
  static getRoomTerminalFreeStorageAmount(room) {
    let freeStorage = 0;
    const terminals = HelpersFind.findStructuresByType(
      room,
      STRUCTURE_TERMINAL
    );
  ...
  static findClosestPath(roomPosition, room, type) {
    return roomPosition.findClosestByPath(
      HelpersFind.findByFindConstant(room, type)
    );
  }
  ...
  static findLinksWithEnergy(room) {
    return HelpersFind.findStructuresByType(room, STRUCTURE_LINK).filter(
      link => link.energy > 0
    );
  }
}
